...
docker-build: ## Build docker image with the manager.
	$(CONTAINER_TOOL) build -f cluster/images/sops-secrets-operator/Dockerfile -t ${IMG} .
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	sed -e '1 s/\(^FROM\)/FROM --platform=\$$\{BUILDPLATFORM\}/; t' -e ' 1,// s//FROM --platform=\$$\{BUILDPLATFORM\}/' cluster/images/sops-secrets-operator/Dockerfile > Dockerfile.cross
	- $(CONTAINER_TOOL) buildx create --name sops-secrets-operator-builder
	$(CONTAINER_TOOL) buildx use sops-secrets-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm sops-secrets-operator-builder
	rm Dockerfile.cross
